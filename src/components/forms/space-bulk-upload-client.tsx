...
import {
  SPACE_TEMPLATE_COLUMNS,
  mapSpaceHeaders,
  getSpaceMissingColumns,
  validateSpaceRow,
  markSpaceFileDuplicates,
  markSpaceDbDuplicates,
} from "@/lib/bulk-upload-spaces";
import type { ParsedSpaceRow } from "@/lib/bulk-upload-spaces";
import {
...
const DATA_COLUMNS = ["name", "floor", "area", "description"] as const;
...
export function SpaceBulkUploadClient({
  organizations,
  showOrgSelect,
  defaultOrgId,
}: Props) {
  ...
  const handleDownloadTemplate = useCallback(() => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([SPACE_TEMPLATE_COLUMNS]);
    ws["!cols"] = SPACE_TEMPLATE_COLUMNS.map((col) => ({
      wch: col === "설명" ? 30 : col === "전용면적(m²)" ? 15 : 12,
    }));
    XLSX.utils.book_append_sheet(wb, ws, "공간목록");
    XLSX.writeFile(wb, "공간_일괄등록_템플릿.xlsx");
  }, []);
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const missing = getSpaceMissingColumns(fileHeaders);
        if (missing.length > 0) {
        ...
        const parsed: ParsedSpaceRow[] = jsonData.map((raw, i) => {
          const mapped = mapSpaceHeaders(raw);
          const { parsed: validData, errors } = validateSpaceRow(mapped);
        ...
        markSpaceFileDuplicates(parsed);

        // Check DB duplicates
        const effectiveOrgId = targetOrgId || defaultOrgId || "";
        if (effectiveOrgId) {
          try {
            const existing = await getExistingSpaceNames(effectiveOrgId);
            markSpaceDbDuplicates(parsed, new Set(existing));
          } catch {
            toast.error("기존 호실명 확인 중 오류가 발생했습니다.");
          }
        }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (toInsert.length === 0) {
      toast.error("등록할 공간이 없습니다.");
      return;
    }

    setLoading(true);
    try {
      const result = await bulkCreateSpaces(effectiveOrgId, toInsert);
      toast.success(`${result.inserted}건의 공간이 등록되었습니다.`);
      router.push("/spaces");
    } catch (err) {
      toast.error(
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground">
                Excel 템플릿을 다운로드하여 공간 정보를 입력하세요
              </span>
            </div>
