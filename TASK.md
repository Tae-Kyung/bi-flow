...
### 3.1 총괄 대시보드 (Super Admin)

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 3.1.1 | KPI 카드 | 총 입주기업 수, 평균 입주율, 이번달 신규 입주/퇴거 | ⬜ |
| 3.1.2 | 기관별 비교 차트 | 기관별 입주율 비교, 기업 상태 분포 차트 (recharts) | ⬜ |
| 3.1.3 | 계약 만료 예정 목록 | 30/60/90일 이내 만료 예정 계약 테이블 | ⬜ |
| 3.1.4 | 최근 활동 로그 | 입주 신청, 계약 생성, 퇴거 등 타임라인 | ⬜ |

### 3.2 기관 대시보드 (Org Admin)

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 3.2.1 | 기관 KPI 카드 | 입주율, 계약 만료 예정, 졸업 기업 수 | ⬜ |
| 3.2.2 | 입주율 추이 차트 | 최근 12개월 입주율 변동 라인 차트 | ⬜ |
| 3.2.3 | 처리 필요 항목 | 신규 입주 신청, 계약 갱신 대상, 퇴거 진행 중 목록 | ⬜ |

### 3.3 입주기업 대시보드 (Tenant)

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 3.3.1 | 계약 정보 요약 | 현재 계약 기간, 배정 호실, 계약 잔여일 | ⬜ |
| 3.3.2 | 공지사항 | 기관 공지사항 목록 | ⬜ |

### 3.4 Realtime 연동

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 3.4.1 | Supabase Realtime 구독 | 대시보드 KPI 실시간 업데이트 | ⬜ |

### 3.5 Phase 3 검증

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 3.5.1 | 대시보드 데이터 정합성 | 실제 DB 데이터와 대시보드 표시 값 일치 확인 | ⬜ |
| 3.5.2 | 역할별 접근 테스트 | 각 역할로 로그인 시 해당 대시보드만 표시되는지 확인 | ⬜ |
...
| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 4.1.1 | 정산 규칙 설정 UI | 기관별 정산 파라미터 커스텀 설정 (단가, 관리비, 배분 방식, 부과 항목) | ⬜ |
| 4.1.2 | 정산 규칙 스키마 확장 | `org_settings` JSONB에 기관 고유 정산 규칙 필드 추가 | ⬜ |

### 4.2 임대료·관리비 산출

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 4.2.1 | 산출 로직 구현 | DB Function: 기관 설정 기반 임대료·관리비 산출 | ⬜ |
| 4.2.2 | 관리비 배분 로직 | 면적 비례, 균등, 커스텀 배분 방식 지원 | ⬜ |
| 4.2.3 | 고지서 일괄 생성 | 특정 월의 전체 활성 계약에 대해 고지서 일괄 생성 | ⬜ |

### 4.3 고지서·수납 관리

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 4.3.1 | 고지서 목록/상세 | 월별·상태별 필터, 일괄 발행, 상세 조회 | ⬜ |
| 4.3.2 | 수납 확인 처리 | Org Admin 수동 납부 확인, 미납 자동 감지 | ⬜ |
| 4.3.3 | 이메일 발송 연동 | Resend API로 고지서/미납 알림 발송 | ⬜ |
| 4.3.4 | 수납 현황 리포트 | 월별 수납률, 미납 금액 요약 | ⬜ |

### 4.4 Phase 4 검증

| # | 태스크 | 상세 | 상태 |
|---|--------|------|------|
| 4.4.1 | 정산 E2E | 기관 설정 → 고지서 생성 → 수납 확인 → 미납 처리 | ⬜ |
